// File: OrderManagement.jsx (Hoàn chỉnh)
...
// ⭐ Sửa: Import hook useSocket từ thư mục '../../hooks/useSocket'
import { useSocket } from '../../hooks/useSocket'; // Sử dụng hook đã định nghĩa
...
const OrderManagement = () => {
    ...
    const { socket, isConnected } = useSocket(); // Lấy socket và trạng thái kết nối
    ...
    useEffect(() => {
        ...
        const handleCustomerNotify = (data) => {
            ...
            // Hiển thị thông báo (nếu đó là cập nhật từ admin khác hoặc cập nhật real-time)
    ...
    }, [socket, openUpdateDialog, selectedOrder]); // Dependencies để đảm bảo listener sử dụng state mới nhất
    ...
    // ✨ Xử lý xem chi tiết đơn hàng (lấy detail)
    ...
    return (
        <Box sx={{ p: 4, zoom: 1.1 }}>
            ...
            {/* {isConnected ? (
                <Chip label="" color="success" size="small" sx={{ mb: 2 }}/>
            ) : (
                <Chip label="" color="error" size="small" sx={{ mb: 2 }}/>
            )} */}
